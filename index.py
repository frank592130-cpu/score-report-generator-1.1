...
def generate_copy_list():
    ...
    wb = openpyxl.Workbook(); ws = wb.active; ws.title = "補習班貼上專用"
...
def build_excel(students_data, exam_lines, ths):
    ...
    wb = openpyxl.Workbook(); ws = wb.active; ws.title = "成績報表"

    # --- 欄位寬度精確控制 ---
    for b in range(3):
        base = b * 4 + 1
        ws.column_dimensions[get_column_letter(base)].width = 9       # 姓名 (A, E, I)
        ws.column_dimensions[get_column_letter(base + 1)].width = 4.5 # 選擇 (B, F, J) 盡量縮短
        ws.column_dimensions[get_column_letter(base + 2)].width = 4.5 # 非選 (C, G, K) 盡量縮短
        ws.column_dimensions[get_column_letter(base + 3)].width = 7.5 # 總分 (D, H, L) 不縮短，確保平均兩位數顯示正常
...
<head>
    ...
    <style>
        body { background-color: #020617; background-image: radial-gradient(at 0% 0%, #1e1b4b 0, transparent 50%), radial-gradient(at 100% 0%, #312e81 0, transparent 50%), radial-gradient(at 50% 100%, #0f172a 0, transparent 50%); background-attachment: fixed; min-height: 100vh; color: #f8fafc; font-family: 'Inter', sans-serif; }
        .glass-card { background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(12px); border: 1px solid rgba(255, 255, 255, 0.1); }
        .btn-main { background: linear-gradient(135deg, #6366f1 0%, #a855f7 100%); transition: all 0.3s ease; }
...
<body class="p-6 md:p-12 flex justify-center">
    <div class="max-w-4xl w-full space-y-8">
        <header class="text-center">
            <h1 class="text-4xl font-black tracking-tighter bg-gradient-to-r from-indigo-400 to-pink-400 bg-clip-text text-transparent">SCORE REPORT GENERATOR</h1>
        </header>
        <div class="glass-card rounded-2xl p-8 space-y-6 shadow-2xl">
            <form id="main-form" action="/generate" method="post" class="space-y-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="space-y-1 text-center"><label class="text-[10px] text-slate-500 font-bold">A++</label><input type="number" step="0.1" id="th_app" name="th_app" value="93.2" class="w-full bg-slate-950/50 border border-slate-800 rounded-lg p-2 text-center"></div>
                    <div class="space-y-1 text-center"><label class="text-[10px] text-slate-500 font-bold">A+</label><input type="number" step="0.1" id="th_ap" name="th_ap" value="85.7" class="w-full bg-slate-950/50 border border-slate-800 rounded-lg p-2 text-center"></div>
                    <div class="space-y-1 text-center"><label class="text-[10px] text-slate-500 font-bold">A</label><input type="number" step="0.1" id="th_a" name="th_a" value="76.2" class="w-full bg-slate-950/50 border border-slate-800 rounded-lg p-2 text-center"></div>
                    <div class="space-y-1 text-center"><label class="text-[10px] text-slate-500 font-bold">B++</label><input type="number" step="0.1" id="th_bpp" name="th_bpp" value="67.1" class="w-full bg-slate-950/50 border border-slate-800 rounded-lg p-2 text-center"></div>
                </div>
    ...
    <script>
        ...
        fileInput.onchange = async () => {
            ...
            if (d.students) { d.students.forEach(newSt => { if(!studentsData.some(s => s.name === newSt.name)) studentsData.push(newSt); }); refreshUI(); }
        };
        ...
        function renderList() {
            ...
            studentsData.forEach((s, i) => {
                ...
                row.innerHTML = `<span class="text-sm font-medium">${s.name} (選: ${s.x})</span><input type="number" step="0.5" value="${s.y}" onchange="updateY(${i}, this.value)" class="w-16 bg-slate-950 border border-slate-700 rounded p-1 text-center text-xs text-emerald-400 outline-none">`;
